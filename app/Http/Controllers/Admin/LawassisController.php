...
class LawassisController extends Controller
{
    //
    public function index(Request $request)
    {
        ...
        if ($created_at) {
            $basements = MovieBasement::where("created_at", "like", "%" . $created_at . "%")->where('type','lawassist')->orderBy("sort",
                "desc")->paginate(20);
            return view("admin.lawassis.index", compact("basements"));
        }
        if ($title) {
            $basements = MovieBasement::where("title", "like", "%" . $title . "%")->where('type','lawassist')->orderBy("sort",
                "desc")->paginate(20);
            return view("admin.lawassis.index", compact("basements"));
        }

        $basements = MovieBasement::where('type','lawassist')->orderBy("sort", "desc")->paginate(20);
        return view("admin.lawassis.index", compact("basements"));
    }
    ...
    public function create(Request $request)
    {
        ...
        return view("admin.lawassis.create", compact('user_id'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->to("/admin/lawassis/")->with("message", "创建成功");
    ...
    public function edit($basementId, Request $request)
    {
        ...
        return view('admin.lawassis.edit', compact('basement','picurls'));
    }
    public function update($basementId, Request $request)
    {
        ...
        return redirect()->to("/admin/lawassis");
    }
}
