...
class Movie extends Model
{
    ...
    public function progressDailyData()
    {
        return $this->hasMany(ProgressDailyData::class,'FMOVIEID','FID');
    }

    public function notices()
    {
        return $this->hasMany(Notice::class, 'FMOVIEID', 'FID');
    }


    public function dailyNoticesFiles()
    {
        return NoticeExcel::whereIn('FNOTICEEXCELID',
            $this->notices()->where('FNOTICEEXCELTYPE', 10)->selectRaw('distinct FID')->pluck('FID')->all()
        )->get();
    }

    public function prepareNoticesFiles()
    {
        return NoticeExcel::whereIn('FNOTICEEXCELID',
            $this->notices()->where('FNOTICEEXCELTYPE', 20)->selectRaw('distinct FID')->pluck('FID')->all()
        )->get();
    }
    ...
    public function hadLocked()
    {
       return MovieLock::where('movie_id',$this->FID)->exists();
    }
}
