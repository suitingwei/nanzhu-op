...
class ProgressDailyData extends Model
{
    ...
    private function updateMovieRestProgressData($totalMovieProgressData)
    {
        ...
        list($restScene, $restPage, $pastDay, $restDay) = $this->calculateRestProgressData($totalMovieProgressData,
            $totaldata);

        list($averageScene, $averagePage, $needDailyPage, $needDailyScene) = $this->calculateNeedProgressData($totalMovieProgressData,
            $pastDay, $restDay, $restPage, $restScene);
    ...
    {
        $totalMovieProgressData = ProgressDailyGroupData::getMovieTotalProgressDataTillDate($this->FMOVIEID,
            $this->FDATE);
     ...
     * 每日数据拍摄组的数量
     */
    public function notEmptyGroupData()
    {
        return $this->groupData
            ->where('FPLANSCENE', '!=', 0)
            ->orWhere('FDAILYSCENE', '!=', 0)
            ->orWhere('FPLANSCENE', '!=', 0)
            ->orWhere('FDAILYPAGE', '!=', 0)
            ->get();
    }

    public function groupData()
    {
        $this->hasMany(ProgressDailyGroupData::class, 'FDAILYDATAID', 'FID');
    }
}
